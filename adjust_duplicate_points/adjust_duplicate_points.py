...
class AdjustDuplicatePoints:
    ...
    def run(self):
        ...
        # layer = self.iface.activeLayer()
        self.dlg.label_4.setText(str(len(selection)))
        ...
        self.dlg.show()
        # self.value = 0
        # self.dlg.dial.valueChanged.connect(self.dlg.spinBox.setValue)
        ...
        if result:
            ...
            # print ('Original Value: ' + str(self.myang))
            # if 0 <= self.myang <= 90:
                # self.myang = self.myang + 270
            # elif 90 < self.myang <=360:
                # self.myang = self.myang - 90
            # print ('new value: ' + str(self.myang))
            # layer = self.iface.activeLayer()
            # selection = layer.selectedFeatures()
            #if layer.wkbType()==QGis.WKBPoint:
            #    print ('Layer is a point layer')
            print('Objects Selected: ' + str(len(selection)))
            new_geom = []
            n = 0
            #angle = 30
            # angle = 300
            ...
            #    new_geom.append(QgsPoint(new_x, new_y))
            #    gPnt = QgsGeometry.fromPointXY(QgsPointXY(new_x, new_y))
                # print ('done')
                field_index = feature.fields().indexFromName('HAS_DUPLIC')
                
            #    layer.changeAttributeValue(feature.id(), field_index, 'No')
